...
class AdminDashboardView(AdminRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        for doctor in doctors:
            doctor.earned = Booking.objects.filter(
                doctor=doctor, 
                status="completed"
            ).aggregate(
                total=Sum('doctor__profile__price_per_consultation')
        ...
        for patient in patients:
            latest_appointment = Booking.objects.filter(patient=patient).order_by('-appointment_date').first()
            patient.last_visit = latest_appointment.appointment_date if latest_appointment else None
            patient.total_paid = Booking.objects.filter(
                patient=patient,
                status="completed"
            ).aggregate(
                total=Sum('doctor__profile__price_per_consultation')
